...
numbers(4 digit-> exclusive)
    Armstrong sayi:herhangi bir 3 basamakli sayinin rakamlarinin kupleri
     toplami o sayiyi veriyorsa sayi  Armstrong sayidir
       ...
       Amstrog sayisini bulan bir method create ediniz.
    ...
    public static void main(String[] args) {
        ...
        armstrong(sayi);
        girilenSayiyaKadarOlanArmstrongSayilar(sayi); //recursive yaptik
    ...
    private static void armstrong(int sayi) {
        ...
        int girilenSayi=sayi; //sayiyi sayinin kupler toplami ile kiyasalamak icin

        while (sayi>0) {
            basamaktakiSayi = sayi % 10; //rakam rakam ayirdik sayi
            sayininKuplerToplami += (basamaktakiSayi * basamaktakiSayi * basamaktakiSayi);
            sayi = sayi / 10;
        }
        if (girilenSayi==sayininKuplerToplami) {
            System.out.println("girilenSayi armtrong sayidir = " + girilenSayi);

        }else System.out.println("girilen sayi armtrong sayi degildir = " +girilenSayi);
